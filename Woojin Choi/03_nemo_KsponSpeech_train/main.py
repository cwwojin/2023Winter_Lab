...
from preprocess.preprocess import preprocess, preprocess_test_data
...
def log_info(opt):
    ...
    print("Dataset Split : %s" % opt.split)
...
def main():
    ...
    if opt.split == 'train' :
        ...
        audio_paths, transcripts = preprocess_test_data(opt.dataset_path, opt.preprocess_mode)
    else :
        raise ValueError(opt.split)

    if opt.output_unit == 'character':
        #generate_character_labels(transcripts, opt.vocab_dest)
        generate_character_script(audio_paths, transcripts, opt.savepath, opt.split)
